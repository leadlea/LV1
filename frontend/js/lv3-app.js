/**
 * Lv3 カリキュラム実行 - プロジェクトリーダーシップシナリオ形式
 * セッション管理、出題→回答→採点→レビューの5ステップフロー制御
 * 全ステップ完了時のみ /lv3/complete を呼び出してDB保存
 */
const Lv3App = (() => {
  const SESSION_KEY = "ai_levels_lv3_session";
  ...
  /** Check Lv2 pass status; redirect if not passed */
  async function checkLv2Gate() {
    let sessionId = null;
    try {
    ...
    try {
      ...
      if (!data.levels || !data.levels.lv3 || !data.levels.lv3.unlocked) {
  ...
  function cacheDom() {
    ...
    els.stepsContainer = document.getElementById("lv3-steps");
  }
  ...
  function updateStepIndicators(currentIdx) {
    const items = els.stepsContainer.querySelectorAll(".lv3-steps__item");
    items.forEach((item, i) => {
      item.classList.remove("lv3-steps__item--active", "lv3-steps__item--done");
      if (i < currentIdx) item.classList.add("lv3-steps__item--done");
      else if (i === currentIdx) item.classList.add("lv3-steps__item--active");
    });
  }
  ...
  function renderFinal(session) {
    ...
    els.finalTitle.textContent = allPassed ? "Lv3 合格！" : "Lv3 結果";
  ...
  async function start() {
    ...
    try {
      ApiClient.hideError();
      const data = await ApiClient.lv3Generate(session.session_id);
  ...
  async function submitAnswer() {
    ...
    try {
      ApiClient.hideError();
      const result = await ApiClient.lv3Grade(session.session_id, question.step, question, answer);
  ...
  async function completeSession() {
    showSection("loading");
    document.querySelector(".lv3-loading-text").textContent = "結果を保存しています...";
  ...
  async function init() {
    const passed = await checkLv2Gate();
    if (!passed) return;
    bindEvents();
    start();
  }
